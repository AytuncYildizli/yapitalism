...
- Pre-chain ledger files require `relayproof ledger migrate --source OLD --output NEW` before new appends; RelayProof will not silently rewrite them. Migration is non-destructive: it writes and verifies a new chained ledger and leaves the source unchanged.
