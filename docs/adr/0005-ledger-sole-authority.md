...
Yapitalism can expose receipts through CLI summaries, future search indexes, or other read models. If any projection can also become writable authority, receipt state can diverge. Actor identity, source, target, session, command, and delivery identifiers are also different dimensions; collapsing them makes routing metadata look like proof authority.

## Decision

The verified append-only JSONL event ledger is Yapitalism's sole local authority. Status, receipt summaries, manifests, search indexes, and future databases are rebuildable projections only.
...
`yapitalism ledger manifest` reports only bounded authority metadata: schema/projection versions, validity, sequence range, event and command counts, and chain head. It does not expose command IDs, targets, event payloads, or terminal content.
...
- Yapitalism does not gain task routing, handoff buses, agent memory, dashboards, or multi-tenant control-plane behavior from this decision.
